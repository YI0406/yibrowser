...
enum _DirectDownloadChoice { open, download }
...
''';
  ...
  Future<void> _onYtOptionsChanged() async {
    ...
    final hlsUrl = repo.ytHlsUrl.value;
    final ytVideoId = _cachedYoutubeInfo?.videoId;
    ...
    IconData iconForEntry(Object entry) {
      if (entry is _YtHlsOption) {
        return Icons.cloud_download;
      }
      final option = entry as YtStreamOption;
      switch (option.type) {
        case YtOptionType.audioOnly:
    ...
    String titleForEntry(Object entry) {
      if (entry is _YtHlsOption) {
        return context.l10n('browser.youtube.option.type.hls');
      }
      final option = entry as YtStreamOption;
      final quality =
          option.qualityLabel ??
    ...
    String subtitleForEntry(Object entry) {
      if (entry is _YtHlsOption) {
        return context.l10n('browser.youtube.option.hlsHint');
      }
      final option = entry as YtStreamOption;
    ...
    try {
      await _showPointerInterceptBottomSheet(
        ...
        builder: (sheetContext) {
          ...
          final int totalItems = opts.length + (hlsUrl != null ? 1 : 0);
          final double desiredHeight =
              headerHeight + itemExtent * totalItems.toDouble();
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ListTile(
                            ...
                            leading: Icon(iconForEntry(entry)),
                            title: Text(
                              titleForEntry(entry),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                            ...
                            onTap: () async {
                              ...
                              if (entry is _YtHlsOption) {
                                await AppRepo.I.enqueueYoutubeHlsManifest(
                                  entry.url,
                                ...
                                final o = entry as YtStreamOption;
  ...
  Future<void> _showYoutubeDownloadOptions(
    ...
    if (info == null ||
        (info.options.isEmpty && info.hlsManifestUrl == null)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          duration: const Duration(seconds: 2),
          content: Text(context.l10n('browser.youtube.error.noStreams')),
    ...
    AppRepo.I.ytHlsUrl.value = info.hlsManifestUrl;
  ...
  void _onPendingOpenDownloadsSheet() {
    final token = repo.pendingOpenDownloadsSheet.value;
    if (token == null) {
      return;
    }
    repo.pendingOpenDownloadsSheet.value = null;
    if (!mounted) {
      return;
    }
  ...
  void _onPendingOpenFavoritesPage() {
    final token = repo.pendingOpenFavoritesPage.value;
    if (token == null) {
      return;
    }
    repo.pendingOpenFavoritesPage.value = null;
    if (!mounted) {
      return;
    }
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _onPendingOpenHistoryPage() {
    final token = repo.pendingOpenHistoryPage.value;
    if (token == null) {
      return;
    }
    repo.pendingOpenHistoryPage.value = null;
    if (!mounted) {
      return;
    }
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _onPendingOpenTabManager() {
    final token = repo.pendingOpenTabManager.value;
    if (token == null) {
      return;
    }
    repo.pendingOpenTabManager.value = null;
    if (!mounted) {
      return;
    }
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void initState() {
    ...
    repo.pendingOpenDownloadsSheet.addListener(_onPendingOpenDownloadsSheet);
    repo.pendingOpenFavoritesPage.addListener(_onPendingOpenFavoritesPage);
    repo.pendingOpenHistoryPage.addListener(_onPendingOpenHistoryPage);
    repo.pendingOpenTabManager.addListener(_onPendingOpenTabManager);
  ...
  void dispose() {
    ...
    repo.pendingOpenDownloadsSheet.removeListener(_onPendingOpenDownloadsSheet);
    repo.pendingOpenFavoritesPage.removeListener(_onPendingOpenFavoritesPage);
    repo.pendingOpenHistoryPage.removeListener(_onPendingOpenHistoryPage);
    repo.pendingOpenTabManager.removeListener(_onPendingOpenTabManager);
                      ...
                      InAppWebView(
                        ...
                        onUpdateVisitedHistory: (
                          ...
                          if (url != null) {
                            ...
                            if (!isBlank) {
                              ...
                              // Update history early so fast multi-clicks still keep order.
                        ...
                        shouldOverrideUrlLoading: (
                          ...
                          try {
                            ...
                            if (url != null && _looksLikeDirectDownloadUrl(url)) {
                              bool isMainFrame = true;
                              try {
                                final dynamic rawAction = navigationAction;
                                isMainFrame = rawAction.isForMainFrame ?? true;
                              } catch (_) {}
                              if (isMainFrame) {
                                bool isLink = true;
                                try {
                                  final navType = navigationAction.navigationType;
                                  isLink = navType == NavigationType.LINK_ACTIVATED;
                                } catch (_) {}
                                if (isLink) {
                                  final handled = await _offerDownloadForNavigation(url);
                                  if (handled) {
                                    return NavigationActionPolicy.CANCEL;
                                  }
                                }
  ...
  Future<void> _promptAddDownloadUrl() async {
    final urlCtrl = TextEditingController();
    final result = await _showPointerInterceptDialog<_DirectDownloadChoice>(
      builder: (dialogContext) {
        return AlertDialog(
          ...
          actions: [
            ...
            FilledButton(
              onPressed:
                  () => Navigator.pop(dialogContext, _DirectDownloadChoice.download),
              child: Text(dialogContext.l10n('common.add')),
            ),
          ],
        );
    ...
    if (result != _DirectDownloadChoice.download) {
      return;
    }
    ...
    final normalized =
        ...
        (raw.startsWith('http://') || raw.startsWith('https://')
            ? raw
            : 'https://$raw');
    final validated = _normalizeHttpUrl(normalized);
    if (validated == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    await AppRepo.I.enqueueDownload(validated);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        duration: const Duration(seconds: 1),
  ...
  Future<bool> _offerDownloadForNavigation(String url) async {
    if (!mounted) {
      return false;
    }
    final choice = await _showPointerInterceptDialog<_DirectDownloadChoice>(
      builder: (_) {
        return AlertDialog(
          title: Text(context.l10n('browser.dialog.detectDownloadable.title')),
          content: Column(
          ...
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, _DirectDownloadChoice.open),
              child: Text(context.l10n('common.open')),
            ),
            FilledButton(
              onPressed:
                  () =>
                      Navigator.pop(context, _DirectDownloadChoice.download),
              child: Text(context.l10n('common.download')),
            ),
          ],
        );
    ...
    if (choice == _DirectDownloadChoice.download) {
      await _confirmDownload(url, skipPrompt: true);
      return true;
    }
    return false;
                            ...
                            return Column(
                              children: [
                                _BottomSheetHeader(
                                  ...
                                  trailing: IconButton(
                                    icon: const Icon(Icons.add),
                                    tooltip: context.l10n('common.add'),
                                    onPressed: _promptAddDownloadUrl,
                                  ),
                                ),
                                Expanded(
                                  child: Center(
  ...
  Widget _buildDownloadTile(DownloadTask t) {
    ...
    final bool isProcessing =
        state == 'processing' || (t.extra?['isConverting'] == true);
    final bool isDone = state == 'done';
    final bool isError = state == 'error';
    final bool isConverting = (t.extra?['isConverting'] == true);

    final bool showProgress = isDownloading || isQueued || isPaused || isProcessing;
    ...
    if (showProgress) {
      ...
      subtitleChildren.add(
        Padding(
          ...
          child: Row(
            mainAxisAlignment:
                (progressLabel != null && percentText != null)
        ...
        children: [
          if (isDownloading)
            IconButton(
              ...
              tooltip: context.l10n('media.state.paused'),
              onPressed: () async {
          ...
          if (isPaused)
            IconButton(
              icon: const Icon(Icons.play_arrow),
              tooltip: context.l10n('media.state.downloading'),
              onPressed: () async {
                await AppRepo.I.resumeTask(t);
              },
            ),
          if (!isDone)
            IconButton(
