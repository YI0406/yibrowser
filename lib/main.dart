...
void main() async {
  ...
  try {
    await bg.FileDownloader().configure(
      iOSConfig: [(bg.Config.resourceTimeout, const Duration(hours: 6))],
    );
  } catch (e) {
...
class _RootNavState extends State<RootNav>
  ...
  static const MethodChannel _deepLinkChannel = MethodChannel('app.deep_link');
  static const MethodChannel _shareBridgeChannel = MethodChannel(
    'app.share_bridge',
  );
  ...
  void initState() {
    ...
    pages = [
      ...
      HomePage(
        ...
        onOpenDownloads: () {
          AppRepo.I.pendingOpenDownloadsSheet.value = Object();
          setState(() {
            index = 2;
          });
        },
        onOpenFavorites: () {
          AppRepo.I.pendingOpenFavoritesPage.value = Object();
          setState(() {
            index = 2;
          });
        },
        onOpenHistory: () {
          AppRepo.I.pendingOpenHistoryPage.value = Object();
          setState(() {
            index = 2;
          });
        },
        onOpenTabs: () {
          AppRepo.I.pendingOpenTabManager.value = Object();
          setState(() {
            index = 2;
          });
        },
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    AppRepo.I.setAppInForeground(state == AppLifecycleState.resumed);
  ...
  void _handleDeepLink(String raw) {
    ...
    if (scheme.startsWith('sharemedia')) {
      unawaited(_handleShareDeepLink());
      return;
    }
    if (scheme != 'yibrowser') return;
  ...
  Future<void> _handleShareDeepLink() async {
    try {
      final media = await ShareHandler.instance.getInitialSharedMedia();
      if (media == null) {
        debugPrint('[Share] Share deep link received but no media payload');
        await _attemptNativeShareConsume(source: 'deepLink');
        return;
      }
      final incoming = _incomingSharesFromSharedMedia(media);
      debugPrint(
        '[Share] Deep link received with ${incoming.length} item(s)',
      );
      if (incoming.isEmpty) {
        await _attemptNativeShareConsume(source: 'deepLink');
        return;
      ...
      debugPrint('[Share] Deep link import failed: $err');
    }
  }
  ...
  void _initShareHandling() {
    ...
    if (Platform.isIOS) {
      unawaited(_syncShareBridgeMetadata());
    }
    ...
    _sharedMediaSubscription = shareHandler.sharedMediaStream.listen(
      (SharedMedia media) {
        ...
        if (incoming.isEmpty) {
          unawaited(_attemptNativeShareConsume(source: 'stream'));
          return;
        }
        _importAndPresentSharedMedia(incoming);
      },
    ...
    shareHandler
        ...
        .then((media) {
          if (media == null) {
            ...
            unawaited(_attemptNativeShareConsume(source: 'initial'));
            return;
          }
          final incoming = _incomingSharesFromSharedMedia(media);
          debugPrint(
  ...
  Future<void> _syncShareBridgeMetadata() async {
    try {
      await _shareBridgeChannel.invokeMethod('syncShareMetadata');
    } catch (err) {
      debugPrint('[Share] Share bridge metadata sync failed: $err');
    }
  }

  Future<void> _attemptNativeShareConsume({required String source}) async {
    if (!Platform.isIOS) {
      return;
    }
    List<IncomingShare> incoming = const <IncomingShare>[];
    try {
      final raw = await _shareBridgeChannel.invokeMethod('consumeSharedItems');
      incoming = _incomingSharesFromNative(raw);
    } catch (err) {
      debugPrint('[Share] Share bridge consume failed: $err');
      return;
    }
    debugPrint(
      '[Share] Share bridge returned ${incoming.length} item(s) for $source',
    );
    if (incoming.isEmpty) {
      return;
    }
  ...
  List<IncomingShare> _incomingSharesFromNative(dynamic raw) {
    if (raw is! List) {
    ...
    for (final entry in raw) {
      ...
      final path =
          (entry['path'] is String) ? (entry['path'] as String).trim() : '';
      if (path.isEmpty) {
        continue;
      }
      final displayName =
          (entry['displayName'] is String)
              ? (entry['displayName'] as String).trim()
              : null;
      final relativePath =
          (entry['relativePath'] is String)
              ? (entry['relativePath'] as String).trim()
              : null;
      final typeHint =
          (entry['type'] is String) ? (entry['type'] as String).trim() : null;
      incoming.add(
        IncomingShare(
          path: path,
          typeHint: typeHint?.isEmpty == true ? null : typeHint,
          relativePath:
