...
class _RateSnapshot {
  ...
  const _RateSnapshot({
    ...
    required this.anchorBytes,
    required this.anchorTimestamp,
    required this.speed,
  });

  factory _RateSnapshot.initial(int bytes, DateTime timestamp) {
...
String _mediaStateLabel(DownloadTask t) {
  final isHls = t.kind == 'hls';
  final l10n = LanguageService.instance;
  if (t.extra?['isConverting'] == true || t.state == 'processing') {
...
class _MediaPageState extends State<MediaPage>
  ...
  List<_TaskActionOption> _taskActionOptions(
    ...
    final premiumUnlocked = PurchaseService().isPremiumUnlocked;
    final lockHide = !hiddenContext && !premiumUnlocked;
    return [
      _TaskActionOption(
        key: 'rename',
        icon: Icons.edit,
                      ...
                      .map(
                        (opt) {
                          ...
                          return ListTile(
                            ...
                            title: Text(opt.label, style: style),
                            onTap: () async {
                              if (locked) {
  ...
  Future<void> _handleTaskAction(
    ...
    if (action == 'edit-export') {
      ...
      final sourcePath = await AppRepo.I.ensureEditableMediaPath(
        task.savePath,
        suggestedName: task.name ?? path.basename(task.savePath),
      );
      if (!mounted) return;
      try {
          ...
          MaterialPageRoute(
            builder:
                (_) => MediaSegmentExportPage(
                  ...
                  displayName: task.name ?? path.basename(sourcePath),
        ...
        if (sourcePath != task.savePath) {
          await AppRepo.I.cleanupTempExportPath(sourcePath);
        }
      }
      if (!mounted) return;
      await AppRepo.I.rescanDownloadsFolder();
    ...
    if (action == 'share') {
      ...
      if (!ok) return;
      if (AppRepo.I.pathExistsSync(task.savePath)) {
  ...
  Widget _buildTaskTile({
    ...
    if (isDone) {
      try {
        if (_isDirectoryPath(task.savePath)) {
          displayBytes = math.max(displayBytes, task.received);
        } else {
    ...
    final actionByKey = {for (final opt in actionOptions) opt.key: opt};
    ...
    final tile = ListTile(
      ...
      subtitle: Column(
        ...
        children: [
          ...
          if (isConverting && (task.total ?? 0) > 0)
            () {
              final total = task.total ?? 0;
              final ratio = (task.received / total.toDouble()).clamp(
                0.0,
              ...
              : Row(
                ...
                children: [
                  ...
                  PopupMenuButton<String>(
                    ...
                    onSelected:
                        (value) async {
                          final opt = actionByKey[value];
                          if (opt != null &&
                              opt.requiresPremium &&
                              !PurchaseService().isPremiumUnlocked) {
                            await PurchaseService().showPurchasePrompt(
                              context,
                              featureName: context.l10n('media.action.hide'),
                            );
                            return;
                          }
                          await _handleTaskAction(
    ...
    return Dismissible(
      key: ValueKey('media_${task.savePath}'),
      direction: DismissDirection.startToEnd,
      confirmDismiss: (_) => _confirmDeleteTask(task),
      onDismissed: (_) => AppRepo.I.removeTasks([task]),
  ...
  double? _directSpeedFor(DownloadTask task) {
    ...
    if (prev == null) {
      _directSpeedSnaps[key] = _RateSnapshot.initial(task.received, now);
      return null;
    }

    int anchorBytes = prev.anchorBytes;
    DateTime anchorTimestamp = prev.anchorTimestamp;
    double? speed = prev.speed;
    final int delta = task.received - anchorBytes;
    final int elapsedMs = now.difference(anchorTimestamp).inMilliseconds;

    const int minWindowMs = 800;
    const int stagnationMs = 2500;

    if (task.received < anchorBytes) {
      anchorBytes = task.received;
      anchorTimestamp = now;
      speed = null;
    } else if (delta > 0 && elapsedMs >= minWindowMs) {
      final double instantaneous = delta / (elapsedMs / 1000.0);
      if (speed != null) {
        speed = (speed * 0.6) + (instantaneous * 0.4);
      } else {
        speed = instantaneous;
      }
      anchorBytes = task.received;
      anchorTimestamp = now;
    } else if (delta <= 0 && elapsedMs >= stagnationMs) {
      speed = null;
      anchorBytes = task.received;
      anchorTimestamp = now;
    }

    _directSpeedSnaps[key] = _RateSnapshot(
      bytes: task.received,
      timestamp: now,
  ...
  // _stateLabel moved to top-level helper for reuse by favorites list.
  ...
  Future<bool> _confirmDeleteTask(DownloadTask task) async {
    final name = task.name ?? path.basename(task.savePath);
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Text(context.l10n('media.dialog.deleteItem.title')),
          content: Text(
            context.l10n(
              'media.dialog.deleteItem.message',
              params: {'name': name},
            ),
            maxLines: 3,
  ...
  OutlinedButton buildHideButton() => OutlinedButton(
      onPressed:
          _selected.isEmpty || !PurchaseService().isPremiumUnlocked
              ? null
    ...
    if (resolvedType == 'video' || resolvedType == 'audio') {
      ...
      if (candidates != null) {
        ...
        for (final item in candidates) {
          ...
          final exists = AppRepo.I.pathExistsSync(item.savePath);
      ...
      builder: (context, tasks, _) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final name = task.name ?? path.basename(task.savePath);
            final fileLength =
                AppRepo.I.pathExistsSync(task.savePath)
                    ? (_isDirectoryPath(task.savePath)
                        ? task.received
            ...
            if (task.state.toLowerCase() == 'downloading' || isConverting) {
                      ...
                      MaterialPageRoute(
                        builder:
                            (_) => MediaSegmentExportPage(
                              sourcePath: sourcePath,
                              displayName:
                                  task.name ?? path.basename(sourcePath),
                              mediaType: resolvedType,
                              initialDuration: task.duration,
                            ),
                      ),
