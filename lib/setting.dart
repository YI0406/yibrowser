...
class _SettingPageState extends State<SettingPage> {
  ...
  void initState() {
    super.initState();
    AppRepo.I.cacheSizeBytes.addListener(_handleCacheSizeChanged);
    _refreshCacheSize();
    _loadUaMode();
    _loadSearchEngine();
  ...
  @override
  void dispose() {
    AppRepo.I.cacheSizeBytes.removeListener(_handleCacheSizeChanged);
  ...
  void _handleCacheSizeChanged() {
    if (!mounted) return;
    final bytes = AppRepo.I.cacheSizeBytes.value;
    if (bytes < 0) return;
    setState(() {
      _cacheSize = AppRepo.I.formatBytes(bytes);
    });
  }
  ...
  Future<void> _refreshCacheSize() async {
    ...
    if (!mounted) return;
    setState(() {
      _cacheSize = repo.formatBytes(bytes);
    });
  }
