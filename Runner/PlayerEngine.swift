...
final class PlayerEngine: NSObject, FlutterStreamHandler, AVPictureInPictureControllerDelegate, AVPlayerViewControllerDelegate {
  ...
  private func handle(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "previewAt":
      ...
      let generator = AVAssetImageGenerator(asset: previewAsset)
    ...
    case "previewFileAt":
      guard let args = call.arguments as? [String: Any],
            ...
            let url = resolveURL(from: path) else {
        result(nil)
        return
      }
      let positionMs: Double
      ...
      asset.loadValuesAsynchronously(forKeys: ["tracks"]) {
        ...
        guard status == .loaded, !asset.tracks(withMediaType: .video).isEmpty else {
          DispatchQueue.main.async {
            result(nil)
          }
          return
        ...
        generator.generateCGImagesAsynchronously(forTimes: [NSValue(time: targetTime)]) { [weak self] _, image, _, status, error in
          if status == .succeeded, let cgImage = image {
            let uiImage = UIImage(cgImage: cgImage)
            let data = uiImage.jpegData(compressionQuality: 0.7) ?? uiImage.pngData()
            DispatchQueue.main.async {
    ...
    case "metadataForFile":
      ...
      asset.loadValuesAsynchronously(forKeys: ["duration", "tracks"]) {
        ...
        let durationMs = durationStatus == .loaded ? self.milliseconds(from: asset.duration) : nil
        var width: Double?
        var height: Double?
