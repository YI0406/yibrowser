...
final class BackgroundDownloadManager: NSObject, FlutterStreamHandler, URLSessionDownloadDelegate {
  ...
  private func handle(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "start":
      ...
      ioQueue.async {
        if let existing = self.task(for: id) {
          let info = self.decodeTaskInfo(existing.taskDescription) ?? [:]
          let existingDestination = info["destination"] ?? dest

          let resumableStates: Set<URLSessionTask.State> = [.running, .suspended]
          if resumableStates.contains(existing.state),
             existingDestination == dest {
            var updatedInfo = info
            updatedInfo["destination"] = dest
            updatedInfo["allowCellular"] = allowCellular ? "1" : "0"
            updatedInfo["allowExpensive"] = allowExpensive ? "1" : "0"
            updatedInfo["allowConstrained"] = allowConstrained ? "1" : "0"

            if let encoded = self.encodeTaskInfo(updatedInfo) {
              existing.taskDescription = encoded
            }
            existing.priority = URLSessionTask.highPriority
            if #available(iOS 11.0, *) {
              existing.earliestBeginDate = nil
            }
            if existing.state == .suspended {
              existing.resume()
            }
            self.activeTasks[id] = existing
  ...
  private func task(for id: String) -> URLSessionDownloadTask? {
    if let existing = activeTasks[id] as? URLSessionDownloadTask {
      return existing
    }
    guard let session = session else { return nil }
    var match: URLSessionDownloadTask?
    let semaphore = DispatchSemaphore(value: 0)
    session.getAllTasks { tasks in
      match = tasks.compactMap { $0 as? URLSessionDownloadTask }.first { task in
        guard let info = self.decodeTaskInfo(task.taskDescription),
              let taskId = info["id"] else {
          return false
        }
        return taskId == id
      }
